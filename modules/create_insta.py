...
from modules.utility_methods import store_credentials
...
class AccountCreator():
    ...
    def createaccount(self, account_info, proxy=None):
        ...
        print(config.Config['chromedriver_path'])
        driver = webdriver.Chrome(
            chrome_options=chrome_options, executable_path=config.Config['chromedriver_path'])
        ...
        # store_credentials(account_info)
    ...
    def creation_config(self, account_info):
        try:
            if self.use_local_ip_address == False:
                if self.use_custom_proxy == False:
                    for i in range(0, config.Config['amount_of_account']):
                        if len(self.sockets) > 0:
                            ...
                            try:
                                self.createaccount(
                                    current_socket, account_info)
                            except Exception as e:
                                print('Error!, Trying another Proxy {}'.format(
                                    current_socket))
                                self.createaccount(
                                    current_socket, account_info)
                    ...
                    with open(config.Config['proxy_file_path'], 'r') as file:
                        ...
                        for proxy in content:
                            ...
                            if amount_per_proxy != 0:
                                print("Creating {} amount of users for this proxy".format(
                                    amount_per_proxy))
                                for i in range(0, amount_per_proxy):
                                    try:
                                        self.createaccount(proxy, account_info)
                ...
                for i in range(0, config.Config['amount_of_account']):
                    try:
                        self.createaccount(account_info)
                    except Exception as e:
...
def create_account_on_insta(account_info):
    account = AccountCreator(
        config.Config['use_custom_proxy'], config.Config['use_local_ip_address'])
    account.creation_config(account_info)
